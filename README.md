# 京投数字法务

北京市基础设施投资有限公司（京投公司）法务合规智能助手

## 功能特性

- 💬 **多轮对话**：支持与AI进行连续对话，记忆上下文
- 📎 **多文档上传**：支持上传txt、md、pdf、docx格式文档
- 🔍 **RAG检索增强**：基于向量检索的智能问答
- 🛡️ **京投法务体系**：内置完整的京投公司法务合规知识库

## 系统架构

```
jingtou-digital-legal/
├── backend/
│   ├── app.py              # FastAPI主应用
│   ├── config.py           # 配置文件
│   ├── llm_client.py       # 私有化大模型客户端
│   ├── embeddings.py       # 文档向量化处理
│   ├── retrieval.py        # RAG检索逻辑
│   └── requirements.txt   # Python依赖
├── frontend/
│   └── index.html          # 前端页面
├── data/
│   └── knowledge_base/     # 知识库文档
└── README.md
```

## 快速启动

### 1. 安装依赖

```bash
cd jingtou-digital-legal/backend
pip install -r requirements.txt
```

### 2. 配置大模型

系统已配置使用您提供的私有化大模型API：
- 模型：Qwen (启元)
- API地址：https://chatai.bii.com.cn/open-api/llm/k8s/qwen/v1/chat/completions

如需修改，请编辑 `backend/config.py` 中的 `LLM_CONFIG`。

### 3. 启动服务

```bash
cd backend
python app.py
```

服务将在 http://localhost:8000 启动

### 4. 访问系统

打开浏览器访问 http://localhost:8000

## 核心能力

### 1. 制度法律审核
为京投公司规章制度的法律审核提供全流程指引。
- 合法性：与现行法律法规、国资委规定、公司章程一致
- 合规性：符合合规管理体系要求，嵌入合规控制点
- 风险可控性：审批权限明确、流程闭环、责任可追溯
- 体系兼容性：与现有制度不冲突、职责边界清晰

### 2. 合同管理
为京投公司合同全生命周期管理提供审核指引和风险提示。
- 六部门协作审核流程
- 合同审核"三个100%"之一：合同协议文本法律审核率100%
- 重大合同提前3个工作日通知法律合规部参与谈判

### 3. 诉讼仲裁案件管理
为京投公司系统诉讼仲裁案件处理提供流程指引和时限要求。
- 案件分类与处理主体
- 重大案件包案制
- 关键时限管理

### 4. 合规管理
为京投公司"四位一体"合规体系建设提供咨询支持。
- "三张清单"机制
- 合规重点领域
- 专项合规指引

### 5. 法务合规公文写作
按照京投公司标准模板起草7类法务合规公文。

## 使用示例

### 咨询问题
```
"三个100%具体指什么？"
"合同审核的流程是什么？"
"重大案件的判断标准是什么？"
```

### 公文写作
```
"帮我起草一份法治工作总结"
"写一份合规季报"
```

## 技术栈

- **后端**：FastAPI + Python
- **向量化**：Sentence-Transformers + FAISS
- **大模型**：Qwen (启元私有化部署)
- **前端**：原生HTML/CSS/JavaScript

## 注意事项

⚠️ 本系统仅供参考，涉及重大决策、复杂诉讼等事项时，需法律合规部正式出具法律意见。

---

北京市基础设施投资有限公司 © 2024
